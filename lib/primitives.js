...
class Submission {
  ...
  static fromPacket(msg) {
    ...
    */ 
    ...
    subm.nonce2 = msg.params[2];
    subm.ts = msg.params[3];
...
class Job {
  ...
  check(sid, subm) {
    ...
    const ts = subm.ts;
    const nonce2 = Buffer.from(subm.nonce2, 'hex');
    const proof = this.attempt.getProof(nonce, ts, nonce2, this.mask);
    return proof;
  }
  ...
  toJSON() {
    ...
    const extraNonce = Buffer.alloc(24);
    const [mask ,maskHash] = this.attempt.randomMask();
    this.mask = mask;

    return [
      this.id,
      Date.now(),
      this.attempt.getHeader(0, Date.now(), extraNonce, maskHash).toString('hex'),
      this.attempt.target.toString('hex'),
      this.attempt.height,
      maskHash.toString('hex')
    ];
  }
}
