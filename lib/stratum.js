...
class Stratum extends EventEmitter {
  ...
  addJob(job) {
    if (this.jobs.size >= Stratum.MAX_JOBS) this.removeJob(this.jobs.head);
  ...
  async handleSubmit(conn, msg) {
    ...
    //if (subm.ts > now + 7200) {
      ...
      //return;
    //}
    this.logger.info("client sid isset before check %s %s",conn.sid,subm);
    
    const share = job.check(conn.sid, subm);
   
    const diffTargetBits = util.targetFromDifficulty(conn.difficulty);
    let diffTarget = common.getTarget(diffTargetBits);
    let OGPoolDiff = diffTarget;
    const difficulty = common.getDifficulty(share.powHash());
    //leaving these comments for debugging.
    //despite working i feel like we'll revisit this at some point
    //this.logger.debug('target     %s',job.target.toString('hex'));
    //this.logger.debug('difftarget %s',diffTarget.toString('hex'));
    if(job.target.compare(diffTarget) > 0){
      diffTarget = job.target; //use actual block target if its lower than static difficulty
    }
    //last bits of comment nastiness
    //this.logger.debug('so we set difftarget to %s',diffTarget.toString('hex'));
    //this.logger.debug(' and pow hash isset to  %s',share.powHash().toString('hex'));
    //this.logger.debug('and compare result is   %s',share.powHash().compare(diffTarget));
    if(share.powHash().compare(OGPoolDiff) > 0 && share.powHash().compare(job.target) > 0){
      this.logger.debug(
        "Client submitted a low share of %d, hash=%s, ban=%d (%s).",
        difficulty,
        share.powHash().toString('hex'),
        conn.banScore,
        conn.id()
      );
    ...
    this.logger.debug(
      ...
      difficulty,
      share.height,//share.rhash(),
      conn.id()
    );

    let error;

    if (share.powHash().compare(job.target) <= 0) {
      const block = job.commit(share);
      error = await this.addBlock(conn, block);
    }
    else{
      this.logger.warning("didnt verify share vs target. share %s  target %s",share.powHash().toString('hex'),job.target.toString('hex'));
    }
